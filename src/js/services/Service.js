...
const PICKING_THE_ORDER_TIME = 5;
...
const realMiliseconds = data.output?.minutes.real;

let delay = 0;
export default function timeToDelay() {
    if (poweredOn) {
        delay = (Number(programMinutes) / Number(realMiliseconds)) * 60000;
    ...
    for (let i = 0; i < numberOfOrders; i++) {
        ...
        let currentTime = ((PICKING_THE_ORDER_TIME + (distance * 2 - 1)) * numberOfDeliveriesForOrder);
    ...
    if (totalTime > MINUTES_PER_DAY) {
        numberOfDrones = Math.ceil(totalTime / MINUTES_PER_DAY);
